...
class ControllerExtensionHbapps extends Controller {
	...
	public function install() {
		...
		if ($this->validate()) {
			...
			$this->response->redirect($this->url->link('extension/hbapps', 'token=' . $this->session->data['token'], true));
		}
	...
	public function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/hbapps', 'token=' . $this->session->data['token'], true)
		);
		...
		if ($files) {
			foreach ($files as $file) {
				...
				$store_data[] = array(
					...
					'edit'   => $this->url->link('hbapps/' . $extension, 'token=' . $this->session->data['token'] . '&store_id=0', true),
				...
				foreach ($stores as $store) {
					$store_data[] = array(
						'name'   => $store['name'],
						'edit'   => $this->url->link('hbapps/' . $extension, 'token=' . $this->session->data['token'] . '&store_id=' . $store['store_id'], true),
				...
				$data['extensions'][] = array(
					...
					'install'   => $this->url->link('extension/hbapps/install', 'token=' . $this->session->data['token'] . '&extension=' . $extension, true),
					'uninstall' => $this->url->link('extension/hbapps/uninstall', 'token=' . $this->session->data['token'] . '&extension=' . $extension, true),
		...
		$this->response->setOutput($this->load->view('extension/hbapps', $data));
	}
